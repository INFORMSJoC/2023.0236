...
#   ub = [upper_bound(x), upper_bound(y)]
#   @variable(m, w)
#   @constraint(m, xy >= lb[1]*yz + lb[2]*xz - lb[1]*lb[2]*z)
#   @constraint(m, xy >= ub[1]*yz + ub[2]*xz - ub[1]*ub[2]*z)
#   @constraint(m, xy <= lb[1]*yz + ub[2]*xz - lb[1]*ub[2]*z)
#   @constraint(m, xy <= ub[1]*yz + lb[2]*xz - ub[1]*lb[2]*z)
# end
...
function conv_tri(m,x_lift,x1,x2,x3,λ)
   ...
   # println("ub: ", ub)
   # println("ex_pt: ", ex_pt)
   @constraint(m, x_lift == sum(ex_pt[i]*λ[i] for i=1:8))
   @constraint(m, x1 == (λ[1] + λ[2] + λ[5] + λ[6])*lb[1] + (λ[3] + λ[4] + λ[7] + λ[8])*ub[1])
   @constraint(m, x2 == (λ[1] + λ[3] + λ[5] + λ[7])*lb[2] + (λ[2] + λ[4] + λ[6] + λ[8])*ub[2])
   @constraint(m, x3 == (λ[1] + λ[2] + λ[3] + λ[4])*lb[3] + (λ[5] + λ[6] + λ[7] + λ[8])*ub[3])
   @constraint(m, sum(λ[i] for i=1:8) == 1)
...
function conv_tri_on_off(m, x_lift, x1, x2, x3, λ, z, x3_bd)
   ...
   @constraint(m, x1 <= (λ[1] + λ[2] + λ[5] + λ[6])*lb[1] + (λ[3] + λ[4] + λ[7] + λ[8])*ub[1] + ub[1] * (1 - z))
   @constraint(m, x1 >= (λ[1] + λ[2] + λ[5] + λ[6])*lb[1] + (λ[3] + λ[4] + λ[7] + λ[8])*ub[1] + lb[1] * (1 - z))
   @constraint(m, x2 <= (λ[1] + λ[3] + λ[5] + λ[7])*lb[2] + (λ[2] + λ[4] + λ[6] + λ[8])*ub[2] + ub[2] * (1 - z))
   @constraint(m, x2 >= (λ[1] + λ[3] + λ[5] + λ[7])*lb[2] + (λ[2] + λ[4] + λ[6] + λ[8])*ub[2] + lb[2] * (1 - z))

   # @constraint(m, x1 == λ_hat[1, 1] * lb[1] + λ_hat[1, 2] * ub[1] + (λ[1] + λ[2] + λ[5] + λ[6])*lb[1] + (λ[3] + λ[4] + λ[7] + λ[8])*ub[1])
   # @constraint(m, x2 == λ_hat[2, 1] * lb[2] + λ_hat[2, 2] * ub[2] + (λ[1] + λ[3] + λ[5] + λ[7])*lb[2] + (λ[2] + λ[4] + λ[6] + λ[8])*ub[2])

   @constraint(m, x3 == (λ[1] + λ[2] + λ[3] + λ[4])*lb[3] + (λ[5] + λ[6] + λ[7] + λ[8])*ub[3])
   @constraint(m, sum(λ[i] for i = 1:8) == z)
...
function conv_tri_sum(m,x_lift,y_lift,x1,x2,x3,y3,λ_x,λ_y)
   ...
   val = [lb_x[1]*lb_x[2],
          ...
          ub_x[1]*lb_x[2],
          ub_x[1]*ub_x[2]]
   @constraint(m, sum(λ_x[i]*val[i] for i=1:8) == sum(λ_y[i]*val[i] for i=1:8))
...
function conv_tri_sum_rlt_1(m,x_lift,y_lift,x1,x2,x3,y3,λ_x,λ_y,x3_lift_i,x3_lift_j,y3_lift_i,y3_lift_j,λ_x3,λ_y3)
   ...
   ex_pt = [lb[1]*lb[3],
            ...
            ub[1]*ub[3],
            ub[1]*ub[3]]
   @constraint(m, x3_lift_i == sum(ex_pt[i]*λ_x3[i] for i=1:8))
   ...
   ex_pt = [lb[2]*lb[3],
            ...
            lb[2]*ub[3],
            ub[2]*ub[3]]
   @constraint(m, x3_lift_j == sum(ex_pt[i]*λ_x3[i] for i=1:8))
   ...
   ex_pt = [lb[1]*lb[3],
            ...
            ub[1]*ub[3],
            ub[1]*ub[3]]
   @constraint(m, y3_lift_i == sum(ex_pt[i]*λ_y3[i] for i=1:8))
   ...
   ex_pt = [lb[2]*lb[3],
            ...
            lb[2]*ub[3],
            ub[2]*ub[3]]
   @constraint(m, y3_lift_j == sum(ex_pt[i]*λ_y3[i] for i=1:8))

   @constraint(m, x1  == x3_lift_i + y3_lift_i)
   @constraint(m, x2  == x3_lift_j + y3_lift_j)
...
function conv_tri_sum_rlt_2(m,x_lift,y_lift,x1,x2,x3,y3,λ_x,λ_y,x3_lift,y3_lift,λ_x3,λ_y3)
   ...
   ex_pt = [lb[1]*lb[2]*lb[3],
            ...
            ub[1]*lb[2]*ub[3],
            ub[1]*ub[2]*ub[3]]
   @constraint(m, x3_lift == sum(ex_pt[i]*λ_x3[i] for i=1:8))
   ...
   ex_pt = [lb[1]*lb[2]*lb[3],
            ...
            ub[1]*lb[2]*ub[3],
            ub[1]*ub[2]*ub[3]]
   @constraint(m, y3_lift == sum(ex_pt[i]*λ_y3[i] for i=1:8))

   # RLT constraint
   #mcc(m, x3_lift, x3, x_lift)
   #mcc(m, y3_lift, y3, y_lift)
   @constraint(m, sum(λ_x[i]*val[i] for i=1:8) == x3_lift + y3_lift)

   return m
end

function relaxation_cos(m, cs, td, td_min, td_max, td_u)
   @constraint(m, cs <= 1 - (1-cos(td_u))/(td_u*td_u)*(td^2))
   @constraint(m, cs >= (cos(td_min) - cos(td_max))/(td_min-td_max)*(td - td_min) + cos(td_min))
   return m
end

function relaxation_sin(m, si, td, td_min, td_max, td_u)
   if td_min < 0 && td_max > 0
      @constraint(m, si <= cos(td_u/2)*(td - td_u/2) + sin(td_u/2))
      @constraint(m, si >= cos(td_u/2)*(td + td_u/2) - sin(td_u/2))
   end
   if td_max <= 0
      @constraint(m, si <= (sin(td_min) - sin(td_max))/(td_min-td_max)*(td - td_min) + sin(td_min))
      @constraint(m, si >= cos(td_u/2)*(td + td_u/2) - sin(td_u/2))
   end
   if td_min >= 0
      @constraint(m, si <= cos(td_u/2)*(td - td_u/2) + sin(td_u/2))
...
function relaxation_cos_on_off(m, bp, td_min, td_max, td_u, td_M, cos_min, cos_max)
   ...
   B = (1 - cos(td_u)) / (td_u)^2
   if params["ext_cons"]
      @constraint(m, cs <= cos_max * z)
   end
   @constraint(m, cs >= cos_min * z)
   # can this be integrated?
   #JuMP.@constraint(m, y >= (cos(lb) - cos(ub))/(lb-ub)*(x - lb) + cos(lb))
   if params["ext_cons"]
      # (8a)
      @constraint(m, - cs + A * td <= (A * td_min - cos(td_min)) * z + abs(A) * td_M * (1 - z))
   end

   # (8b)
   if params["ext_cons"]
      @constraint(m, cs <= z - B * (td^2) + B * td_M^2 * (1-z))

      # S = {1} or {2}
      # if A >= 0
      #    @constraint(m, - cs <= (A * td_max - cos(td_min) - A * td_min) * z)
      # else
      #    @constraint(m, - cs <= (A * td_max - cos(td_min) - A * td_max) * z)
      # end
      #
      # @constraint(m, A * td <= (A * td_max - cos(td_min) + cos_max) * z + abs(A) * td_M * (1 - z))
   else
      @constraint(m, cs <= z - B * ((va_i - va_j)^2) + B * td_M^2 * (1-z))
   end
...
function relaxation_sin_on_off(m, bp, td_min, td_max, td_u, td_M, sin_min, sin_max)
    ...
    if params["ext_cons"]
       @constraint(m, si <= sin_max * z)
       @constraint(m, si >= sin_min * z)
    end
    ...
    if td_min < 0 && td_max > 0
      if params["ext_cons"]
         # @constraint(m, si <= cos(td_u/2)*(td - td_u/2) + sin(td_u/2))
         # @constraint(m, si >= cos(td_u/2)*(td + td_u/2) - sin(td_u/2))
          @constraint(m,  si - C * td <= (- C * td_u / 2 + sin(td_u / 2)) * z + C * td_M * (1 - z))
          @constraint(m, -si + C * td <= (- C * td_u / 2 + sin(td_u / 2)) * z + C * td_M * (1 - z))
          # println("(- C * td_u/2 + sin(td_u / 2)): ", (- C * td_u/2 + sin(td_u / 2)))
          # println("C * td_M: ", C * td_M)
          # S = {1} or {2}:
          # @constraint(m, si <= (- C * td_u / 2 + sin(td_u / 2) + C * td_max) * z)
          # @constraint(m, - si <= (- C * td_u / 2 + sin(td_u / 2) - C * td_min) * z)
          # @constraint(m, -C * td <= (- C * td_u / 2 + sin(td_u / 2) - sin_min) * z + (C * td_M) * (1 - z))
          # @constraint(m, C * td <= (- C * td_u / 2 + sin(td_u / 2) + sin_max) * z + (C * td_M) * (1 - z))
       else
          # @constraint(m, si <= cos(td_u/2) * (va_i - va_j - td_u/2) + sin(td_u/2))
          # @constraint(m, si >= cos(td_u/2) * (va_i - va_j + td_u/2) - sin(td_u/2))
          @constraint(m,  si - C * (va_i - va_j) <= (- C * td_u / 2 + sin(td_u / 2)) * z + C * td_M * (1 - z))
          @constraint(m, -si + C * (va_i - va_j) <= (- C * td_u / 2 + sin(td_u / 2)) * z + C * td_M * (1 - z))
       end
    end
    if params["ext_cons"]
       if td_max <= 0
         @constraint(m, si - D * td <= (-D * td_min + sin(td_min)) * z + D * td_M * (1 - z))
         @constraint(m, -si + C * td <= (- C * td_u/2 + sin(td_u / 2)) * z + C * td_M * (1-z))
         @constraint(m, si <= (- D * td_min + sin(td_min) + D * td_max) * z)
         @constraint(m, -D * td <= (- D * td_min + sin(td_min) - sin_min) * z + (D * td_M) * (1 - z))
       end
       if td_min >= 0
         @constraint(m,  si - C * td <= (- C * td_u / 2 + sin(td_u / 2)) * z + C * td_M * (1 - z))
         @constraint(m, -si + D * td <= (D * td_min - sin(td_min)) * z + D * td_M * (1 - z))
         @constraint(m, - si <= (D * td_min - sin(td_min) - D * td_min) * z)
         @constraint(m, -D * td <= (D * td_min - sin(td_min) + sin_max) * z + (D * td_M) * (1 - z))
       end
    end
...
function relaxation_voltage_lnc(m, wr, wi, w_i, w_j, buspair)
   ...
   @constraint(m, sf*st*(cos(phi)*wr + sin(phi)*wi) - vtub*cos(d)*st*w_i - vfub*cos(d)*sf*w_j >=  vfub*vtub*cos(d)*(vflb*vtlb - vfub*vtub))
   @constraint(m, sf*st*(cos(phi)*wr + sin(phi)*wi) - vtlb*cos(d)*st*w_i - vflb*cos(d)*sf*w_j >= -vflb*vtlb*cos(d)*(vflb*vtlb - vfub*vtub))
end
...
function relaxation_voltage_lnc_on_off(m, wr, wi, w_i, w_j, buspair, z, wz_ij, wz_ji)
   ...
   # Voltage Product Relaxation Upper-bound
   @constraint(m, sf*st*(cos(phi)*wr + sin(phi)*wi) - vtub*cos(d)*st*wz_ij - vfub*cos(d)*sf*wz_ji >=  vfub*vtub*cos(d)*(vflb*vtlb - vfub*vtub) * z)
   @constraint(m, sf*st*(cos(phi)*wr + sin(phi)*wi) - vtlb*cos(d)*st*wz_ij - vflb*cos(d)*sf*wz_ji >= -vflb*vtlb*cos(d)*(vflb*vtlb - vfub*vtub) * z)
end
...
function conv_bi_x_li(m, cyc, x_li, expairs, params, x_type, x_li_bar=:x_li_bar)
   ...
   for j in 1:dim
     ...
     if params["separation"]
        push!(beta, @constraint(m, lb[j] * expr_1 + ub[j] * expr_2 == x_li_bar[j]))
     else
        @constraint(m, x_li[j] == lb[j] * expr_1 + ub[j] * expr_2)
     end
   end
   if x_type == "w" && length(cyc) == 4 && params["rotate_4w"]
      expairs_1 = expairs[1:8]
      expairs_2 = expairs[9:end]
      @constraint(m, [ep in expairs_1], x[cyc, ep] == sum(λ[cyc, i] * X[i][ep[1]] * X[i][ep[2]] for i in 1:(2^dim)))
      @constraint(m, [ep in expairs_2], x[cyc, ep] == sum(λ[cyc, i] * X[i][ep[1]] * X[i][ep[2]] * X[i][ep[3]] for i in 1:(2^dim)))
   else
      @constraint(m, [ep in expairs], x[cyc, ep] == sum(λ[cyc, i] * X[i][ep[1]] * X[i][ep[2]] for i in 1:(2^dim)))
   end
   if params["separation"]
      return beta
   end
...
function conv_bi_x_li_on_off(m, cyc, x_li, ub, lb, ub2, lb2, expairs, params, x_type, x_li_bar=:x_li_bar)
   ...
   for j in 1:dim
     ...
     # println("cnt_1: $(cnt_1), total: $(2^dim)")
     # @assert cnt_1 == 2^dim / 2
     @constraint(m, x_li[j] >= lb[j] * expr_1 + ub[j] * expr_2 + lb2[j] * (1 - m[:zc][Tuple(cyc)]))
     @constraint(m, x_li[j] <= lb[j] * expr_1 + ub[j] * expr_2 + ub2[j] * (1 - m[:zc][Tuple(cyc)]))
   end
   if x_type == "w" && length(cyc) == 4 && params["rotate_4w"]
      expairs_1 = expairs[1:8]
      expairs_2 = expairs[9:end]
